 ...
 */
public interface UsersRepo extends JpaRepository<Client, Long> {

    /**
     * Finds a {@link Client} entity by its username.
     *
     * @param name the username to search for
     * @return an {@link Optional} containing the user if found, or empty if no user exists with that name
     */
    Optional<Client> findByName(String name); // Useful for fetching a user by username
}
